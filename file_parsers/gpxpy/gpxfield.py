    ...
    def from_xml(self, parser, node, version):
        ...
        for child in children:
            if parser.get_node_name(child) == 'TrackPointExtension':
                children_tkpt_text = parser.get_children(child)
                for child_tkpt_text in children_tkpt_text:
                    result[parser.get_node_name(child_tkpt_text)]=parser.get_node_data(child_tkpt_text)
            else:
